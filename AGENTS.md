...
- 일반 개발 흐름은 GitHub Issue 생성 또는 기존 Issue 확인 → `dev`에서 `feat/이슈번호-기능명` 생성 → 개발·테스트·빌드 → `dev` 대상 PR 생성이다. `main`을 대상으로 PR을 만들지 않는다.
- 구현 PR은 하나의 GitHub Issue와 연결한다. 기존 Issue가 있으면 재사용하고, 없으면 PR 준비 전에 생성한다.
...
- 사용자가 PR 생성을 요청하면 연결할 GitHub Issue를 확인하거나 생성하고, 브랜치·변경 범위·공식 문서 준수·테스트·빌드·PR 대상 `dev`를 검증한다.
