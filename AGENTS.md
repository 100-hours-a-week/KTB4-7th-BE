...
- PR 제목은 한글로 작성한다. 본문에는 아래 내용을 구체적으로 작성하고 `Closes #이슈번호`를 포함한다.
  - 목적과 작업 범위: 무엇을 해결하며, 포함·제외한 범위는 무엇인지
  - 구현 상세: 변경한 계층·주요 클래스·메서드와 적용한 비즈니스 규칙
  - 테스트: 실행 명령, 정상·예외 검증 항목, 실제 결과
  - API·DB 영향: 변경된 URI·요청·응답·상태 코드, 테이블·컬럼·제약조건 여부
  - 의존 관계·후속 작업: 선행 또는 후속 Issue·PR이 있으면 번호와 이유
