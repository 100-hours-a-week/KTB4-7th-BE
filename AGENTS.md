...
- 일반 개발 흐름은 `dev`에서 `feat/기능명`을 생성하고, `feat/*`에서 개발·테스트·빌드 후 `dev` 대상으로 PR을 만든다. `main`을 대상으로 PR을 만들지 않는다.
...
- PR 제목은 한글로 작성한다. 본문에는 주요 변경사항, 구현 기능, 테스트 내용·결과, API 변경 여부, DB 변경 여부를 포함한다.
...
- 사용자가 PR 생성을 요청하면 브랜치·변경 범위·공식 문서 준수·테스트·빌드·PR 대상 `dev`를 검증한다.
